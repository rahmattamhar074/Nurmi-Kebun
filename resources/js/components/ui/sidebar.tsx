          ...
          {badge &&
            (state !== "collapsed" ? (
              <span
                ...
                className="-translate-y-1/2 absolute inset-y-1/2 right-1.5 h-5.5 w-auto rounded-full bg-green-600 px-2 text-white text-[10px]/5.5 font-semibold"
              >
                {badge}
              </span>
            ) : (
...
const SidebarDisclosurePanel = ({
  ...
  return (
    <DisclosurePanel
      ...
      {...props}
    >
      <div className="col-span-full grid min-w-0 grid-cols-[auto_1fr]">
        {props.children}
      </div>
    </DisclosurePanel>
  );
